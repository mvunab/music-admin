    ...
    <v-row>
      <v-col>
        ...
        <v-row justify="center" align="center" class="mb-6">
          <v-btn icon @click="changeTargetMonth(-1)">
        ...
        <v-card rounded="xl" class="pa-sm-6 pa-4">
          <v-form ref="planFormRef" v-model="isFormValid" @submit.prevent="submitPlanificacion">
            <v-row>
              ...
              <v-col cols="12">
                <v-select
                  v-model="planificacion.selectedSunday"
                  :items="availableSundaysForSelectedMonth"
                  item-title="text"
                  item-value="value"
                  label="Seleccionar Domingo del Servicio"
                  ...
                  variant="outlined"
                  clearable
                  :no-data-text="`No hay Domingos futuros en ${monthYearDisplay(targetPlanMonth)}`"
                ></v-select>
              </v-col>
              ...
              <v-col cols="12">
                <v-select
                  v-model="planificacion.personInCharge"
                  :items="listaPersonasACargo"
                  item-title="name"
                  item-value="id"
                  label="Seleccionar Persona a Cargo (Director)"
              ...
              <v-col cols="12">
                <v-autocomplete
                  v-model="planificacion.selectedMusicians"
                  :items="listaMusicos"
                  item-title="name" 
                  item-value="id"
                  label="Seleccionar Músicos"
                  ...
                  variant="outlined"
                  clearable
                  hint="Selecciona uno o más músicos"
                  persistent-hint
                >
              ...
              <v-col cols="12">
                <v-autocomplete
                  v-model="planificacion.selectedChoir"
                  :items="listaCoro"
                  item-title="name"
                  item-value="id"
                  label="Seleccionar Miembros del Coro"
                  ...
                  variant="outlined"
                  clearable
                  hint="Selecciona uno o más miembros del coro"
                  persistent-hint
                ></v-autocomplete>
              </v-col>
            </v-row>
            ...
            <div v-for="(song, index) in planificacion.songs" :key="index" class="mb-4 pa-3 border rounded">
              <v-row align="center">
                ...
                <v-col cols="3" md="2" class="text-right">
                  <v-btn
                    v-if="planificacion.songs.length > 1"
...
const listaPersonasACargo = ref([
  ...
  { id: 'p3', name: 'Carlos Ruiz (Líder de Alabanza)' },
]);
...
const listaMusicos = ref([
  ...
  { id: 'm3', name: 'Luis Torres', instrument: 'Bajo Eléctrico' },
  { id: 'm4', name: 'Carlos Vargas', instrument: 'Batería' },
]);
...
const listaCancionesSugeridas = ref([
  'Cuán Grande es Él', 'Sublime Gracia', 'En la Cruz', 'Renuévame',
]);
...
const availableSundaysForSelectedMonth = computed(() => {
  ...
  while (dateIterator.getMonth() === month) {
    if (dateIterator.getDay() === 0) { 
      ...
      if (sundayCandidate.getTime() >= today.getTime()) {
        sundays.push({
          text: sundayCandidate.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
...
const submitPlanificacion = async () => {
  ...
  if (valid) {
    ...
    setTimeout(() => {
      loading.value = false;
      snackbar.value = { show: true, message: 'Planificación guardada exitosamente.', color: 'success'};
