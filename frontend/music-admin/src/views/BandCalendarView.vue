    ...
    <v-row>
      <v-col>
        <div class="d-flex justify-space-between align-center mb-6 flex-wrap">
          ...
          <v-btn @click="logout" color="primary" variant="outlined" prepend-icon="mdi-logout">
            Cerrar Sesión
          </v-btn>
        </div>
        ...
        <div v-if="weeklyEnsayoSlots.length > 0">
          <v-row>
            <v-col
              v-for="(slot, index) in weeklyEnsayoSlots"
              :key="index"
              cols="12"
              sm="6" 
              md="4"
              lg="3"
            >
              <v-card class="mb-4 fill-height" rounded="lg" elevation="2" :color="slot.sundayEvent.director === '-' ? 'grey-lighten-3' : undefined">
                ...
                <v-card-text class="py-3">
                  ...
                  <div v-if="slot.sundayEvent.musicians && slot.sundayEvent.musicians.length > 0">
                    <v-chip
                      v-for="(musician, mIndex) in slot.sundayEvent.musicians"
                      :key="`m-${mIndex}`"
                      size="small"
                      label
                      class="mr-1 mb-1"
                ...
                <v-card-text class="py-3">
                  ...
                  <div v-if="slot.sundayEvent.choir && slot.sundayEvent.choir.length > 0">
                    <v-chip
                      v-for="(choirMember, cIndex) in slot.sundayEvent.choir"
                      :key="`c-${cIndex}`"
                      size="small"
                      label
                      class="mr-1 mb-1"
                ...
                <v-card-text class="py-3">
                  ...
                  <div class="text-body-1">
                    ...
                    <span class="text-caption text-grey"> ({{ slot.mondayBefore.toLocaleDateString(undefined, {month: 'short'}) }})</span>
                  </div>
                  <div v-if="slot.mondayRehearsal" class="mt-1">
                    <p class="text-caption font-weight-medium">{{ slot.mondayRehearsal.title || 'Ensayo Programado' }}</p>
                    <p v-if="slot.mondayRehearsal.details" class="text-caption">{{ slot.mondayRehearsal.details }}</p>
                    <p v-if="slot.mondayRehearsal.start" class="text-caption">Hora: {{ formatTime(slot.mondayRehearsal.start) }}</p>
                  </div>
        ...
        <v-card v-if="selectedSlot" rounded="lg">
            ...
            <v-card-text>
                ...
                <div v-if="selectedSlot.mondayRehearsal">
                    ...
                    <p v-if="selectedSlot.mondayRehearsal.start"><strong>Hora:</strong> {{ formatTime(selectedSlot.mondayRehearsal.start) }}</p>
                    <p v-if="selectedSlot.mondayRehearsal.details"><strong>Detalles:</strong> {{ selectedSlot.mondayRehearsal.details }}</p>
                </div>
...
.v-card-item, .v-card-text, .v-card-actions {
  flex-grow: 0;
}
